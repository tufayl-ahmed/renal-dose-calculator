import { compactText, formatNumber, routeDisplayName, selectedRouteDisplayName } from "./format.js";
import { correctCommonLookupTypo } from "./openfda.js";
...
export function buildMissingLabelSpecialResult(patient) {
  ...
  if (/\bfludarabine\b/.test(requestedName) && /\b(?:tablet|tablets|oral)\b/.test(requestedName)) {
    const sourceUrl = "https://dailymed.nlm.nih.gov/dailymed/drugInfo.cfm?setid=ac76aca8-e718-4232-92ab-399166ce9e46";
...
export function buildSpecialDrugResult({ label, patient }) {
  ...
  if (matchesRequested(/\bdocetaxel\b|\btaxotere\b/)) {
    return buildNoAdjustmentSpecialResult({
      ...
      caution:
        "Docetaxel labeling does not provide a simple CrCl dose adjustment; hepatic function and regimen context drive dosing.",
    });
  }
    ...
    if (patient.route === "IV") {
      return buildCrclSpecialResult({
        ...
        dose: {
          ...
          frequency:
            "Trimethoprim-only IV renal guidance was not cleanly matched; verify whether TMP-SMX was intended.",
  ...
  if (
    matchesRequested(
      /\bolmesartan\b.*\bamlodipine\b.*\bhydrochlorothiazide\b|\bolmesartan\b.*\bhydrochlorothiazide\b.*\bamlodipine\b|\bamlodipine\b.*\bolmesartan\b.*\bhydrochlorothiazide\b|\btribenzor\b/
    )
  ) {
  ...
  if (matchesRequested(/\bceftriaxone\b|\brocephin\b/)) {
    return buildNoAdjustmentSpecialResult({
      ...
      band: Number.isFinite(patient.crcl) ? `CrCl ${formatNumber(patient.crcl)} mL/min` : "CrCl not available",
      caution:
        "Ceftriaxone label states usual doses generally do not require renal adjustment; do not exceed 2 g/day when significant renal and hepatic dysfunction coexist.",
    });
  }
    ...
    if (patient.route === "ORAL") {
      return buildCrclSpecialResult({
        ...
        dose: {
          ...
          frequency:
            "Review DailyMed source; inhaled aztreonam products should not be treated as oral renal-dose guidance.",
  ...
  if (matchesRequested(/\bvalacyclovir\b|\bvaltrex\b/)) {
    return {
      ...
      dialysisNote: "",
      importantCautions: ["Valacyclovir renal dosing changes by indication; do not use one generic dose for all uses."],
    };
  }
  ...
  if (matchesRequested(/\bmoxifloxacin\b|\bavelox\b/)) {
    return buildNoAdjustmentSpecialResult({
      ...
      band: Number.isFinite(patient.crcl) ? `CrCl ${formatNumber(patient.crcl)} mL/min` : "CrCl not available",
      caution:
        "Moxifloxacin label describes no meaningful renal pharmacokinetic change, including hemodialysis or CAPD; verify selected label.",
    });
  }
  ...
  if (
    matchesRequested(
      /\b(?:sulfamethoxazole\s+(?:and\s+)?trimethoprim|trimethoprim\s+(?:and\s+)?sulfamethoxazole|bactrim|septra|co-?trimoxazole|tmp\s*[-/]?\s*smx)\b/
    )
  ) {
  ...
  if (matchesRequested(/\bdigoxin\b|\blanoxin\b/)) {
    return {
      ...
      dialysisNote: "",
      importantCautions: ["Digoxin dose should not be selected from CrCl alone."],
    };
  }
  ...
  if (matchesRequested(/\bvancomycin\b/)) {
    return {
      ...
      dialysisNote: "",
      importantCautions: ["Vancomycin dosing is individualized and should not be reduced to a single CrCl dose line."],
    };
  }
  ...
  if (matchesRequested(/\bapixaban\b|\beliquis\b/)) {
    return {
      ...
      dialysisNote: "",
      importantCautions: ["Do not choose apixaban dose from CrCl alone."],
    };
  }
  ...
  if (matchesRequested(/\brivaroxaban\b|\bxarelto\b/)) {
    return {
      ...
      dialysisNote: "",
      importantCautions: ["Do not choose rivaroxaban dose from CrCl alone without the indication."],
    };
  }
  ...
  if (matchesRequested(/\bdabigatran\b|\bpradaxa\b/)) {
    return {
      ...
      dialysisNote: "",
      importantCautions: ["Do not choose dabigatran dose from CrCl alone without indication and interaction review."],
    };
  }
  ...
  if (matchesRequested(/\benoxaparin\b|\blovenox\b/)) {
    return {
      ...
      dialysisNote: "",
      importantCautions: ["Enoxaparin renal dosing should not be shown as one generic dose without indication."],
    };
  }
  ...
  if (matchesRequested(/\bsitagliptin\b|\bjanuvia\b/)) {
    ...
    const dose = egfr === null ? "Review DailyMed source" : egfr >= 45 ? "100 mg" : egfr >= 30 ? "50 mg" : "25 mg";
    const band =
      egfr === null
        ? "eGFR not available"
        : egfr >= 45
    ...
    return {
      ...
      dialysisNote: "",
      importantCautions: ["Sitagliptin renal dosing is based on eGFR; verify current DailyMed label."],
    };
  }
...
function buildCeftazidimeAvibactamDose(crcl) {
  ...
  if (crcl > 50) {
    return {
      ...
      dose: "2.5 g IV",
      frequency: "every 8 hours",
      cautions: ["Use with metronidazole for cIAI per label when indicated."],
    };
  }
  if (crcl >= 31) {
    return {
      status: "dose_found",
      band: "CrCl 31-50 mL/min",
      dose: "1.25 g IV",
      frequency: "every 8 hours",
      cautions: ["Administer after hemodialysis on hemodialysis days if applicable."],
    };
  }
  if (crcl >= 16) {
    return {
      status: "dose_found",
      band: "CrCl 16-30 mL/min",
      dose: "0.94 g IV",
      frequency: "every 12 hours",
      cautions: ["Administer after hemodialysis on hemodialysis days if applicable."],
    };
...
function buildCefiderocolDose(crcl) {
  ...
  if (crcl >= 120) {
    return {
      status: "dose_found",
      band: "CLcr >= 120 mL/min",
      dose: "2 g IV",
      frequency: "every 6 hours",
      cautions: ["Infuse over 3 hours per label."],
    };
  }
  if (crcl >= 60) {
    return {
      status: "dose_found",
      band: "CLcr 60-119 mL/min",
      dose: "2 g IV",
      frequency: "every 8 hours",
      cautions: ["Infuse over 3 hours per label."],
    };
  ...
  return {
    ...
    dose: "0.75 g IV",
    frequency: "every 12 hours",
    cautions: ["Includes intermittent hemodialysis in the cited label; administer after dialysis if applicable."],
  };
}
...
function buildCobicistatDose(crcl) {
  ...
  if (crcl < 70) {
    return {
      ...
      dose: "150 mg",
      frequency:
        "once daily with atazanavir or darunavir and food; not recommended with TDF when CrCl is below 70 mL/min.",
...
function buildOlmesartanAmlodipineHctzDose(crcl) {
  ...
  if (crcl <= 30) {
    return {
      ...
      dose: "Avoid use",
      frequency:
        "Use an alternative antihypertensive plan; hydrochlorothiazide-containing label advises avoiding this renal band.",
    };
  }
  return {
    status: "no_renal_adjustment",
...
function buildEmpagliflozinDose(egfr) {
  ...
  return {
    ...
    cautions: [
      "Use for glycemic control is generally not recommended at low eGFR; other indications may differ by label.",
    ],
  };
}
