...
test("Telegram webhook rejects wrong secret token", async () => {
  const request = new Request("https://renal-dose-calculator-3fl.pages.dev/api/telegram/webhook", {
    method: "POST",
    headers: {
...
test("Telegram webhook ignores callback-only updates", async () => {
  const response = await onRequestPost({
    request: new Request("https://renal-dose-calculator-3fl.pages.dev/api/telegram/webhook", {
      method: "POST",
...
function telegramMessageRequest(text) {
  return new Request("https://renal-dose-calculator-3fl.pages.dev/api/telegram/webhook", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      message: {
